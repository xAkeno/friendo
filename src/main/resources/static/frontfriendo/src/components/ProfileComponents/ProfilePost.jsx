    ...
    <div>
      <div className="grid-cols-3 grid gap-2 [&>img]:rounded-sm mb-5 max-md:px-15">
        {
          Array.isArray(props.data.feed) ? props.data.feed.map((item,index) => (
            item.imageMetaModels.slice(0,1).map((itemz,indez) => (<img key={indez} src={itemz.imageUrl} onClick={() => {setShow(true);setPost(item)}}className="h-75 w-75 max-lg:h-45 max-lg:w-45 cursor-pointer"/>))
      ...
      </div>
      {
        ...
        <div id="popup-modal"  className="flex bg-gray-900/75 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%)] max-h-full">
            <div className='w-[60%] h-[75%] flex gap-2 max-xl:w-[80%] max-md:w-[90%] max-md:h-[60%] max-md:flex-col'>
