...
const feed = (props) => {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-5 mb-5">
      ...
      {
        feed.map((item,index) => (
          <LoadedFeed id={item.id} liker={item.like} imageMetaModels={item.imageMetaModels} context={item.context} created={item.created_At} visibility={item.visibility} account={item.account} updateShowComment={() => {updateShowComment(index)}} index={index} showComment={showComment} comments={item.comments} option={option} updateOption={() => updateOption(index)} loadedAllLiker={item.likeFeed}/>
        ))
      }
    </div>
  )
